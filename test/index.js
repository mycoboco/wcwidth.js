...
test('handles wide strings', (t) => {
  t.strictEqual(wcwidth('한글字的模块テスト'), 18);
  t.end();
});

test('handles wide characters mixed with regular characters', (t) => {
  t.strictEqual(wcwidth('abc 한글字的模块テスト'), 22);
  t.end();
});

test('handles Hangul Jamos', (t) => {
...
test('invalid sequences with surrogate high/low values', (t) => {
...
test('negative custom value for NUL == -1', (t) => {
  t.strictEqual(wcwidth.config({
    nul: -1,
  })('abc\n한글字的模块テスト\0def'), -1);
  t.end();
